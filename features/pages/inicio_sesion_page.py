...
class LoginPage(Page):
    CORREO_ELECTRONICO_INPUT_ID = (MobileBy.ID, "")
    CONTRASEÑA_INPUT_ID = (MobileBy.ID, "")
    BOTON_INGRESAR_ID = (MobileBy.ID, "")
    ...
    MENSAJE_CONTRASEÑA_INCORRECTA_ID = (MobileBy.ID, "")
    ...
    def ingresar_contraseña(self, contraseña):
        contraseña_input = self.find_element(*self.CONTRASEÑA_INPUT_ID)
    ...
    def validar_mensaje_error_contraseña_incorrecta(self, mensaje):
        mensaje_error = self.find_element(*self.MENSAJE_CONTRASEÑA_INCORRECTA_ID).text
        assert mensaje_error == mensaje, f"El mensaje de error '{mensaje_error}' no coincide con '{mensaje}'"
